    ...
    <head>
        ...
        <%-- <script>
               $(document).ready(function () {
                 var urlBase = "/SIRCON/Docente?accion=agregarNota";
                 $("#curso").on('change', function () {
                     var urlCursos = urlBase + "1&curso=" + $(this).val();
                     var notaAux = $("#nota").val();
                     urlSedes = urlSedes + "&nota=" + peliculaAux;
                     if (urlSedes)
                         window.location = urlSedes;
                     return false;
                 });

                $("#alumno").on('change', function () {
                    var urlSalas = urlBase + "2&alumno=" + $(this).val();
                    ...
                    <div class="col-sm-10">
                        <select id="curso" name="curso" class="form-control"
                            ...
                            <c:forEach items="${curso}" var="curso">
                                <option value="${cursos.codCursos}" ${cursos.codCurso == selectedCurso ? 'selected' : ''}>${cursos.nombre}</option>
                            </c:forEach>>
                        </select>
                    </div>                                               
                </div>
                    ...
                    <div class="col-sm-10">
                        <select id="alumno" name="alumno" class="form-control"
                            ...
                            <c:forEach items="${alumno}" var="alumno">
                                <option value="${alumno.codAlumno}" ${alumno.codAlumno == selectedAlumno ? 'selected' : ''}>${alumno.Usuario_dni.nombres}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
